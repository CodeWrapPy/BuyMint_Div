...
def run_seed():
    # BUG FIX #8: No try/except — if seeding crashed halfway (e.g. a duplicate key
    # on the second run), the session was left dirty and the DB ended up with partial
    # data on subsequent calls. Now we rollback cleanly on failure.
    try:
        for p in PRODUCTS:
            product = Product(
                name           = p["name"],
